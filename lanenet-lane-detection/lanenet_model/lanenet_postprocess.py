...
import os
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), 'lanenet-lane-detection')))
import cv2
import numpy as np
import loguru
...
class LaneNetPostProcessor(object):
    ...
    def __init__(self, cfg, ipm_remap_file_path=None):
        ...
        :param ipm_remap_file_path: Path to the IPM remap file (optional)
        """
        # If ipm_remap_file_path is not provided, compute it relative to this file's location
        if ipm_remap_file_path is None:
            ipm_remap_file_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'tusimple_ipm_remap.yml')

        # Verify that the file exists
        assert os.path.exists(ipm_remap_file_path), '{:s} does not exist'.format(ipm_remap_file_path)
