...
class Config(dict):
    ...
    def _load_config_file(config_file_path):
        ...
        print(f"Trying to load config file from: {config_file_path}")
        print(f"Absolute path: {os.path.abspath(config_file_path)}")
        print(f"Does the file exist? {os.path.exists(config_file_path)}")
        if not os.access(config_file_path, os.R_OK):
            raise OSError('Config file: {:s}, can not be read'.format(config_file_path))
        with open(config_file_path, 'r') as f:
            config_content = yaml.safe_load(f)
...
lanenet_cfg = Config(config_path='./lanenet-lane-detection/config/tusimple_lanenet.yaml')